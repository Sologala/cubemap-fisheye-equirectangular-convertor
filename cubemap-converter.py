...
class CubeMapConverter:
    ...
    def __init__(self, master):
        ...
        self.combobox_projection = ttk.Combobox(
            self.mainframe, state='readonly')
        self.combobox_projection['textvariable'] = self.current_projection
        self.combobox_projection['values'] = (
            "Fisheye Equisolid", "Equirectangular")
        self.combobox_projection.grid(
            column=0, columnspan=2, row=2, pady=10, padx=20)
        self.combobox_projection.bind(
            "<<ComboboxSelected>>", self.combobox_updated)

        ttk.Separator(self.mainframe, orient=HORIZONTAL).grid(
            column=0, columnspan=2, row=3, sticky=(W, E), pady=10)
        ...
        ttk.Label(self.mainframe, text="2. Load cube map image", font='bold', justify="left").grid(column=0,
                                                                                                   ...
                                                                                                   padx=20,
                                                                                                   sticky=(W, E))
        self.button_select_cube_map = ttk.Button(
            self.mainframe, text="Select cube map...")
        self.button_select_cube_map['command'] = self.choose_cube_map
        self.button_select_cube_map['state'] = 'disabled'
        self.button_select_cube_map.grid(
            column=0, columnspan=2, row=5, ipady=10, ipadx=15)

        ttk.Separator(self.mainframe, orient=HORIZONTAL).grid(
            column=0, columnspan=2, row=6, sticky=(W, E), pady=10)
        ...
        ttk.Label(self.mainframe, text="3. Provide required information", font='bold', justify="left").grid(column=0,
                                                                                                            ...
                                                                                                            padx=20,
                                                                                                            sticky=(W, E))
        ttk.Label(self.mainframe, text="FOV (deg)", justify="left").grid(
            column=0, row=8, pady=10, padx=20, sticky=W)

        self.field_of_view = StringVar()
        self.entry_fov = ttk.Entry(
            self.mainframe, textvariable=self.field_of_view, width=10)
        ...
        self.entry_size_output_image = ttk.Entry(
            self.mainframe, textvariable=self.size_output_image, width=10)
        self.entry_size_output_image.grid(column=1, row=9, pady=10, sticky=W)

        ttk.Separator(self.mainframe, orient=HORIZONTAL).grid(
            column=0, columnspan=2, row=10, sticky=(W, E), pady=10)
        ...
        ttk.Label(self.mainframe, text="4. Generate the image!", font='bold', justify="left").grid(column=0,
                                                                                                   ...
                                                                                                   padx=20,
                                                                                                   sticky=(W, E))
        self.button_generate_output_image = ttk.Button(
            self.mainframe, text="Generate image!")
        self.button_generate_output_image['command'] = self.create_output_image
        self.button_generate_output_image['state'] = 'disabled'
        self.button_generate_output_image.grid(
            column=0, columnspan=2, row=12, ipady=10, ipadx=15)
        ...
        self.progressbar.grid(column=0, columnspan=2,
                              row=14, pady=15, ipadx=10)

        ttk.Separator(self.mainframe, orient=HORIZONTAL).grid(
            column=0, columnspan=2, row=15, sticky=(W, E), pady=10)
        ...
        self.button_save_image.grid(
            column=0, columnspan=2, row=16, ipady=10, ipadx=15)
        ...
        self.button_exit.grid(column=0, columnspan=2,
                              row=17, ipady=10, ipadx=15, pady=10)
    ...
    def choose_cube_map(self):
        types = [('PNG image', '*.png'), ('BMP image', '*.bmp'),
                 ('JPEG image', '*.jpeg'), ('All files', '*')]
    ...
    def create_output_image(self):
        ...
        if self.current_projection.get() == "Fisheye Equisolid":
            try:
                self.output_image = self.cubemap_to_fisheye(self.cube_map_image, float(
                    self.field_of_view.get()), int(self.size_output_image.get()))
                self.output_image = Image.fromarray(
                    self.output_image.astype('uint8'))
                self.output_image.show()
            except ValueError:
                messagebox.showinfo(
                    message='Please, provide required information', icon='error')
        elif self.current_projection.get() == "Equirectangular":
            self.output_image = self.cubemap_to_equirectangular(
                Image.fromarray(self.cube_map_image.astype('uint8')))
            self.output_image.show()
    ...
    def demo(self):
        self.cube_map_image = Image.open("./cubemap.png")
        self.cube_map_image = np.array(self.cube_map_image)
        self.output_image = self.cubemap_to_fisheye(
            self.cube_map_image, float(180), int(1000))
    ...
    def cubemap_to_fisheye(self, cube_map, fov, output_image_height):
        ...
        import cv2
        r, phi = fisheye.get_spherical_coordinates(
            output_image_height, output_image_height)
        rr = cv2.normalize(r, None, 0, 1, cv2.NORM_MINMAX)
        # cv2.imshow("rr", rr)
        pphi = cv2.normalize(phi, None, 0, 1, cv2.NORM_MINMAX)
        # cv2.imshow("pphi", pphi)

        x, y, z = fisheye.spherical_to_cartesian(r, phi, field_of_view)
        xx = cv2.normalize(x, None, 0, 1, cv2.NORM_MINMAX)
        # print("hhhh", x.shape)
        ...
        for row in range(0, output_image_height):
            for column in range(0, output_image_height):
                if np.isnan(r[row, column]):
                    ...
                    face = fisheye.get_face(
                        x[row, column], y[row, column], z[row, column])
                    u, v = fisheye.raw_face_coordinates(
                        face, x[row, column], y[row, column], z[row, column])
                    xn, yn = fisheye.normalized_coordinates(
                        face, u, v, face_size)
            ...
            self.style.configure('text.Horizontal.TProgressbar',
                                 text='{:.2f} %'.format(self.progress_var.get()))
        ...
        for ycoord in range(0, output_height):
            for xcoord in range(0, output_width):
                corrx, corry, face = equi.cubemap_to_equirectangular(xcoord,
                                                                     ...
                                                                     n)
                output_img.putpixel(
                    (xcoord, ycoord), cube_map.getpixel((corrx, corry)))
